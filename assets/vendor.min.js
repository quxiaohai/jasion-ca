...
class LazyImage extends HTMLImageElement{constructor(){super(),this.selecter="loading"}connectedCallback(){!this.hasAttribute("data-srcset")&&null===this.media||(window.__lazy_i||(window.__lazy_i=new IntersectionObserver(t=>{t.forEach(t=>{t.isIntersecting&&((t=t.target).hasAttribute("data-srcset")?t.handleLazy.call(t):(t.hasAttribute("src")||t.hasAttribute("srcset"))&&t.mediaLazy.call(t),window.__lazy_i.unobserve(t))})},{root:null,threshold:0})),window.__lazy_i.observe(this))}get media(){return this._media=this._media||this.closest(".media"),!this._media&&this.classList.contains("image")&&(this.selecter="__loading__",this._media=this.parentNode),this._media}mediaLazy(){this.complete||this.classList.contains("loaded")||(this.media.classList.add(this.selecter),this.addEventListener("load",()=>{var t=()=>{this.classList.add("loaded"),this.media.classList.remove(this.selecter)};window.requestIdleCallback?window.requestIdleCallback(t,{timeout:150}):setTimeout(t)},!1))}handleLazy(e){if(!e.classList.contains("loaded")){var s=e.getAttribute("data-srcset").split(","),i=e.parentNode.offsetWidth,a=s.length;for(let t=0;t<a;t++){var r=s[t].trim().split(" "),l=parseInt(r[1].trim());if(0===t&&i<l){e.src=r[0].trim();break}if(t===a-1&&l<i){e.src=r[0].trim();break}var r=s[Math.min(t+1,a-1)].trim().split(" "),d=parseInt(r[1].trim());if(l<=i&&i<=d){e.src=r[0].trim();break}}this.mediaLazy.call(this)}}}customElements.define("lazy-image",LazyImage,{extends:"img"});
$heybike.on('DOMContentLoaded', () => $heybike.emit('VENDOR.LOADED'));