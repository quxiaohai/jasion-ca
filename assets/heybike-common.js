class HeybikeCommon {
    ...
    tabSwitch({root, type = 'tab', panel = 'panel', selector = 'active', device, group, nodeList, cb}) {
        ...
        return this.proxy(root, type, ({elem, nodeList}) => {
            ...
            const filter = arr => name ? arr.filter(n => n.dataset[key] === name) : arr;
            const elems = filter([].concat(nodeList[type] || []));
            const panels = filter([].concat(nodeList[panel] || []));
    ...
    async addToCart({items, button, discount}, buyNow = false, cb) {
