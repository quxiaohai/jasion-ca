    ...
    theme.initWhenVisible = (function () {
        class ScriptLoader {
            constructor(callback, delay = 5000) {
                ...
                this.userInteractionEvents = $heybike.isMobile ? ['touchstart'] : ['mousemove', 'keydown', 'wheel'];
...
class StickyHeader extends CustomHeader {
    ...
    beforeInit() {
        ...
        this.isHeaderScrolled = $heybike.ref(false);
        this.isHeaderScrolled2 = $heybike.ref(false);
        this.isHidden = $heybike.ref(false);
        $heybike.watch(this.isHeaderScrolled, (n, o) => {
            if (n) {
        ...
        $heybike.watch(this.isHeaderScrolled2, (n, o) => {
            if (n) {
    ...
    onScrollHandler() {
        ...
        this.isHeaderScrolled.value = scrollTop > this.headerBounds.top + this.firstScrollTop + this.headerBounds.height;
        this.isHeaderScrolled2.value = this.isHeaderScrolled.value && scrollTop > this.headerBounds.top + this.firstScrollTop + this.headerBounds.height * 2;

        if (scrollTop > this.headerBounds.bottom + this.firstScrollTop + 100) {
            this.isHidden.value = scrollTop > this.currentScrollTop;
        } else {
...
class SliderElement extends HTMLElement {
    ...
    get auto() {
        return !!this.hasAttribute('data-auto');
    }

    get duration() {
        return Number(this.getAttribute('data-duration') || 3500);
    }
    ...
    updateButtons() {
        ...
        if (this.auto) {
            this.startAuto(nextDisabled ? (previousDisabled ? 'hidden' : 'disabled') : 'visible');
        }
...
class MediaGallery extends HTMLElement {
    ...
    openZoom(index = 0) {
        ...
        this.loadPhotoswipe().then(photoswipe => photoswipe.loadAndOpen(index, dataSource));
    }
...
class CountdownTimer extends HTMLElement {
    ...
    get isCompact() {
        this._isCompact = typeof this._isCompact === 'boolean' ? this._isCompact : (this.getAttribute('data-compact') === 'true' || (this.getAttribute('data-compact') === 'mobile' && theme.config.mqlSmall));
    ...
    init() {
        ...
        if (this.nodeList.day && this.nodeList.day.value === '0') {
            const node = this.nodeList.day.closest('.countdown__item');
    ...
    calculate() {
        ...
        if (this.nodeList.day) {
            this.nodeList.day.value = days;
            this.nodeList.hour.value = hours;
            this.nodeList.min.value = mins;
            this.nodeList.sec.value = secs;
            return false;
        }
...
class SlideshowElement extends HTMLElement {
    ...
    get cellAlign() {
        return this.getAttribute('cell-align') || 'center';
    }
    ...
    init() {
        ...
        if (this.items.length > 1) {
            this.slider = new Flickity(this, {
                ...
                prevNextButtons: false,
                wrapAround: true,
                cellAlign: $heybike.isMobile ? 'center' : this.cellAlign,
                rightToLeft: theme.config.rtl,
                autoPlay: this.autoplay ? this.speed : false,
...
class SlideElement extends HTMLElement {
    constructor() {
        super();
        $heybike.inView(this, this.init.bind(this), {margin: '600px 0px 600px 0px'});
    }
...
$heybike.on('DOMContentLoaded', () => $heybike.emit('THEME.LOADED'));